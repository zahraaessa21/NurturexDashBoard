// src/pages/admin/AdminInfants.jsx
//
// Full infant CRUD table (mother's name shown inline instead of a
// separate Patients page — admins just need the parent's name here).
// Search by name, filter by date added, edit/delete inline.

import { useEffect, useState, useCallback } from 'react'
import { useNavigate } from 'react-router-dom'
import { Plus, Search, Baby, RefreshCw, AlertCircle, Pencil, Trash2 } from 'lucide-react'
...
const PAGE_SIZE = 10
const EMPTY_FORM = {
  parent_id: '', doctor_id: '', name: '', date_of_birth: '', gender: 'male',
  birth_weight_kg: '', birth_height_cm: '', blood_type: '', notes: '', status: 'monitoring',
}

export default function AdminInfants() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { rows, total } = await infantService.list({ search, status, dateFrom, dateTo, page, pageSize: PAGE_SIZE })
  ...
  useEffect(() => {
    adminService.listDoctorsForSelect({ specialty: 'pediatrics' }).then(setDoctors).catch(() => {})
    patientService.list({ pageSize: 1000 })
  ...
  const submit = async (e) => {
    ...
    setSaving(true)
    try {
      const payload = { ...form, parent_id: form.parent_id || null, doctor_id: form.doctor_id || null }
      if (editing) {
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-500 mt-1">All infants in the platform, with their mother and assigned doctor.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-slate-500 dark:text-zinc-500 border-b border-slate-200 dark:border-zinc-800">
                ...
                <th className="px-5 py-3 font-medium hidden lg:table-cell">Doctor</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
              {loading ? (
                ...
                rows.map(i => (
                  <tr key={i.id} className="hover:bg-slate-50 dark:hover:bg-zinc-900/50 cursor-pointer" onClick={() => navigate(`/admin/infants/${i.id}`)}>
                    ...
                    <td className="px-5 py-3.5 text-slate-600 dark:text-zinc-300 hidden lg:table-cell">
                      {i.doctor?.full_name ? `Dr. ${i.doctor.full_name}` : <span className="text-slate-400">Unassigned</span>}
                    </td>
      ...
      >
        <form onSubmit={submit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Assigned doctor" hint="Pediatricians only">
